...
function ListJob({jobList}) {
    const daysRemaining = Math.max(0, Math.floor((jobList.expiredDate - Date.now()) / (1000 * 60 * 60 * 24)));
    return (
                ...
                {jobList.map((job, index) => (
                    <div key={index} className='flex flex-row bg-white shadow-md p-5 rounded-lg'>
                        ...
                        <div className='flex flex-col w-5/12 gap-2'>
                            <div className="text-[#18191c] text-base font-medium font-['Inter'] leading-normal">{jobList.title}</div>
                            <div className="inline-flex items-center gap-4">
                                <span className="text-[#767f8c] text-sm font-normal font-['Inter'] leading-tight">{jobList.jobType}</span>
                                <span className="text-[#767f8c] text-sm font-normal font-['Inter'] leading-tight">{daysRemaining} days remaining</span>
                            </div>
                        </div>
                        ...
                        <div className='inline-flex gap-1 items-center w-2/12'>
                            <img 
                                src={daysRemaining === 0 ? `/image/CheckCircle.svg` : `/image/XCircle.png`} 
                                className='h-6 w-6' 
                                alt="status"
                            />
                        ...
                        <div className='inline-flex gap-1 items-center w-3/12'>
                            ...
                            <div className="text-[#5e6670] text-sm font-normal font-['Inter'] leading-tight">
                                {jobList.applications.length} Applications
                            </div>
                        </div>
