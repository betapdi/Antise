...
function ListJob({ isSearch }) {
      ...
      try {
        ...
        if (response.data.length === 0) {
          setDialogContent({
            title: "No jobs found!",
            content:
              "There are no jobs that match your search criteria. Please try again.",
            buttonLabel: "Close",
            link: null,
          });
  ...
  const calculateRemainingDays = () => {
    ...
    for (let i = 0; i < jobs.length; i++) {
      const targetDate = new Date(jobs[i].expirationDate);
      const now = new Date();
      const diff = targetDate - now;
      const daysLeft = Math.ceil(diff / (1000 * 60 * 60 * 24));

      // Store the remaining days using the job's id as key
      remainingDaysMap[jobs[i].id] = daysLeft;
  ...
  const { favoriteJobs, removeFavoriteJob, addFavoriteJob } =
  ...
  return (
    ...
    >
      ...
      >
        {isSearch === 1 ? (
          <button
            className="h-12 px-6 py-3 bg-[#0a65cc] rounded-[3px] justify-center items-center gap-3 inline-flex"
            onClick={() => {
              ...
              <div className="flex-col justify-start items-start gap-3.5 inline-flex">
                ...
                <div className="justify-start items-center gap-4 inline-flex">
                  ...
                  <div className="justify-start items-center gap-1.5 flex">
                    ...
                    />
                    <div className="text-[#636a7f] text-sm font-normal font-['Inter'] leading-tight">
                      {remainingDays[job.id]} Days Remaining
                    </div>
                  </div>
                </div>
              </div>
